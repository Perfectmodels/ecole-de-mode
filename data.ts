import type { Course, SiteSettings, TrainingModule } from './types';

// NOTE: This file acts as a mock database. 
// In a real application, this data would be fetched from and updated via an API.
// The CMS will directly mutate these arrays and objects for this demonstration.

export let coursesData: Course[] = [
  // FIX: Changed id from number to string to match Course type
...
export let siteSettingsData: SiteSettings = {
    contact: {
        address: 'Nzeng-Ayong, 6ᵉ arrondissement, Libreville, Gabon',
        phone: '+241 XX XX XX XX',
        whatsapp: '+241 06 66 66 66',
        email: 'contact@ena-mode.ga'
    },
    social: {
        facebook: '#',
        instagram: '#',
...
export let trainingModulesData: TrainingModule[] = [
    {
        // FIX: Changed id from number to string to match TrainingModule type
        id: '1',
        title: 'Les Fondamentaux du Stylisme',
        description: 'Un module complet pour maîtriser les bases du stylisme, de la théorie des couleurs à la création de moodboards.',
        chapters: Array.from({ length: 20 }, (_, i) => ({
            // FIX: Changed chapter id to string to match Chapter type
            id: (i + 1).toString(),
            title: `Chapitre ${i + 1}: Titre du chapitre`,
            content: `Ceci est le contenu détaillé pour le chapitre ${i + 1}. Il couvre les aspects essentiels du sujet avec des exemples pratiques et des exercices.`,
            videoUrl: 'https://www.youtube.com/embed/VHn-COwN7ng'
        }))
    },
    {
        // FIX: Changed id from number to string to match TrainingModule type
        id: '2',
        title: 'Techniques Avancées de Couture',
        description: 'Passez au niveau supérieur avec des techniques de haute couture, des finitions complexes et le travail des matières nobles.',
        chapters: Array.from({ length: 20 }, (_, i) => ({
            // FIX: Changed chapter id to string to match Chapter type
            id: (i + 1).toString(),
            title: `Chapitre ${i + 1}: Titre avancé`,
            content: `Contenu avancé pour le chapitre ${i + 1} du module sur les techniques de couture.`,
        }))
    }
];